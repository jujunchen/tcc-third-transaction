package csdn.itsaysay.we.third.third1;
...
@Service
public class T1FaceService {
	
	
	public String faceAdd(FaceDO face) {
		String url = "http://localhost:8081/face/add";
		...
		if (!Objects.equals(resObject.getStr("code"), "200")) {
			throw new RuntimeException("third1添加人脸失败，错误信息：" + resObject.getStr("msg"));
		}
	...
	public void faceDelete(String faceId) {
		String url = "http://localhost:8081/face/delete/%s";
		String res = HttpUtil.post(String.format(url, faceId), "");
		JSONObject resObject = JSONUtil.parseObj(res);
		
		if (!Objects.equals(resObject.getStr("code"), "200")) {
			throw new RuntimeException("third1删除人脸失败，错误信息：" + resObject.getStr("msg"));
		}
	}
